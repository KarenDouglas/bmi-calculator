...
describe('test that all visible aspects of the BMI Component are rendered after loaded', () => {
    ...
    test('renders main heading and paragraph content', () => {
        // Elements to be tested
         ...
         const paragraph = screen.getByRole('paragraph',{name: 'Better understand your weight in relation to your height using our body mass index (BM) calculator. While BMI is not the sole determinant of a healthy weight, it offers a valuable starting point to evaluate your overall health and well-being.'})
     ...
     test('renders CSS Image and logo',() => {
        // image elements to be tested
        const cssImage = screen.getAllByRole('figure');
        const logo = screen.getAllByAltText('BMI Calculator Logo');
        
        //tests that css image and logo are rendered
        expect(cssImage).toBeDefined();
